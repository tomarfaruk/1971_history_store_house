<?php require_once('../../../private/init.php'); ?>
<?php

$response = new Response();
$errors = new Errors();

if(Helper::is_post()){
    $api_token = Helper::post_val("api_token");
    if($api_token){
        $setting = new Setting();
        $setting = $setting->where(["api_token" => $api_token])->one();

        if(!empty($setting)){
            $page = Helper::post_val("page");
            $videos = new Video();

            if($page){
                $start = ($page - 1) * API_PAGINATION;
                $videos = $videos->where(["admin_id" => $setting->admin_id])->andWhere(["status"=>1])
                    ->not(["featured" => 1])
                    ->greater_eq(['view_count' => $setting->popular_view_count])
                    ->orderBy("id")->desc()
                    ->limit($start, API_PAGINATION)->all();
            }else $videos = $videos->where(["admin_id" => $setting->admin_id])->andWhere(["status"=>1])
                ->not(["featured" => 1])
                ->greater_eq(['view_count' => $setting->popular_view_count])
                ->orderBy("id")->desc()->all();

            if(!empty($videos)) {
                foreach ($videos as $video) {
                    $video->created = $video->days_ago();
                    $video->category = $video->get_category_title();
                    $video->description = null;
                    $video->admin_id = null;
                    $video->status = null;
                }
                $response->create(200, "Success.", $videos);
            }else $response->create(201, "No Video Found.", null);

        }else $response->create(201, "Invalid Api Token", null);
    }else $response->create(201, "No Api Token Found", null);
}else $response->create(201, "Invalid Request Method", null);

echo $response->print_response();

?>
