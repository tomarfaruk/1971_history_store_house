...
if(Helper::is_post()){
    ...
    if($api_token){
        ...
        if(!empty($setting)){
            ...
            $categories = new Img_category();

            if($page){
                $start = ($page - 1) * API_PAGINATION;
                $categories = $categories->where(["admin_id" => $setting->admin_id])
                    ->orderBy("img_cat_id")->desc()->limit($start, API_PAGINATION)->all();
            }else $categories = $categories->where(["admin_id" => $setting->admin_id])->orderBy("img_cat_id")->desc()->all();
